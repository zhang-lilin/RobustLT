...
class ImbalancedCIFAR10(ImbalancedDataset):
    def load_base_dataset(self, **kwargs):
        ...
        self.dataset = torchvision.datasets.CIFAR10(train=True, **kwargs)
...
def load_cifar10lt(
    ...
    train_dataset = ImbalancedCIFAR10(
        base_dataset=base_dataset, validation=validation, imb_rate=imb_rate, imb_type=imb_type,
        seed=seed,transform=transform_train, logger=logger, **data_args,
    )
    if validation:
        transform_test = T.Compose([T.ToTensor()])
        val_dataset = torchvision.datasets.CIFAR10(**data_args, train=True, transform=transform_test)
