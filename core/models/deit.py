...
from timm.models.layers import PatchEmbed, Mlp, DropPath, trunc_normal_, lecun_normal_, StdConv2dSame, DropPath, to_2tuple, trunc_normal_,lecun_normal_
...
def deit(name, logger, num_classes=10, normalize=True, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5), device='cpu'):
    """
    ...
    assert name in SUPPORT_SET
    load_model = SUPPORT_SET[name]

    logger and logger.log(f'--- using normalization {mean}, {std}.')
    patch_size, img_size = 4, 32
    cfgs = {}
    cfgs['num_classes'],cfgs['mean'],cfgs['std'] = num_classes, mean, std
    cfgs['patch'] = patch_size
