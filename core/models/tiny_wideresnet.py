...
class WideResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_conv = nn.Conv2d(num_input_channels, nChannels[0],
                                   kernel_size=3, stride=1, padding=1, bias=False)
        self.layer = nn.Sequential(
            _BlockGroup(num_blocks, nChannels[0], nChannels[1], 1,
                        activation_fn=activation_fn),
            _BlockGroup(num_blocks, nChannels[1], nChannels[2], 2,
                        activation_fn=activation_fn),
            _BlockGroup(num_blocks, nChannels[2], nChannels[3], 2,
                        activation_fn=activation_fn))
        self.batchnorm = nn.BatchNorm2d(nChannels[3], momentum=0.01)
        self.relu = activation_fn(inplace=True)
        # self.logits = nn.Linear(nChannels[3], num_classes)
